...
export default function TextComponent(props) {
	...
	if (!props.readOnly) { 
		if (fullScreen) {
			...
			if (props.editInline) {
				return <textarea  rows={props.rows ? props.rows : 15} cols={props.cols ? props.cols : 50}  type="text"  style={{float: 'left', width: '90%'}}   value={props.value ? props.value : ''} onChange={function(e) {props.onChange(e.target.value)}} />	
		  ...
		  return <span  >
				{fullText ? <>{splitNewlinesToDivs(props.value)}<Button onClick={ function(e) {setFullText(false)}} variant={'warning'} >{'Less Info'}</Button></> : ((props.value && props.value.split) ? props.value.split("\n")[0].replace("."," ") : '')}
