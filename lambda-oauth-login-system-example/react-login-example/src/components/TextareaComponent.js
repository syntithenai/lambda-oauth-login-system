...
export default function TextareaComponent(props) {
	...
	if (!props.readOnly) { 
		...
		<Form.Control  style={{float:'left', width:'90%'}} as='textarea' rows={props.rows > 0 ? props.rows : 4}  type='text' value={props.value ? props.value : ''} onChange={function(e) {props.onChange(e.target.value)}} />		
		</span>
	 } else {
